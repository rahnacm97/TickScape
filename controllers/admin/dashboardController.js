      ...
      switch (filterType) {
        ...
        case "custom":
          startDate = req.query.startDatee
            ? new Date(req.query.startDatee)
            : new Date(now.setHours(0, 0, 0, 0));
          endDate = req.query.endDatee
            ? new Date(req.query.endDatee)
            : new Date(now);
          endDate.setHours(23, 59, 59, 999);
          break;
        default:
  ...
  try {
    ...
    if (filter === "yearly") {
      ...
      const salesMap = new Map(
        salesData.map((item) => [item._id, item.totalSales])
      );
      labels = allMonths;
      ...
      labels = weekRanges.map(
        (range) => `${year}-${month}-${range.start}-${range.end}`
      );
      const salesMap = new Map(
        salesData.map((item) => [item._id, item.totalSales])
      );

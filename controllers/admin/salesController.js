...
const fetchSalesData = async (
  ...
  const totalCouponCount =
    totalCouponUsers.length > 0 ? totalCouponUsers[0].totalCouponApplied : 0;
  ...
  const totalReturnedOrders = await Order.countDocuments({
    status: "Returned",
    ...query,
  });
...
const downloadExcelReport = async (req, res) => {
  try {
    ...
    // Calculate total returned and cancelled items
    const totalReturnedItems = salesData.order.reduce((sum, order) => {
      return (
        sum +
        order.orderedItems
          .filter((item) => item.orderStatus === "Returned")
          .reduce((itemSum, item) => itemSum + item.quantity, 0)
      );
    }, 0);

    const totalCancelledItems = salesData.order.reduce((sum, order) => {
      return (
        sum +
        order.orderedItems
    ...
    worksheet.columns = [
      ...
      { key: "activeFinalAmount", width: 15 },
      { key: "status", width: 15 },
    ...
    salesData.order.forEach((order) => {
      const activeAmount = order.orderedItems
        .filter(
          (item) =>
    ...
    const summaryHeaderRow = worksheet.addRow([
      ...
      "Total Orders (Non-Cancelled/Returned)",
  ...
  try {
    ...
    const summaryTable = {
      ...
      rows: [
        ...
        ["Total Returned Items", totalReturnedItems],
        ["Total Cancelled Items", totalCancelledItems],
